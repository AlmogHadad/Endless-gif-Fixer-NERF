...
class NonlinearApproximator(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(2, self.hidden_layers[0])
        self.fc2 = nn.Linear(self.hidden_layers[0], self.hidden_layers[1])
        self.fc3 = nn.Linear(self.hidden_layers[1], 3)  # Output has 2 dimensions for y and z
...
def plot_camera_positions_with_direction(json_data):
    ...
    return np.column_stack((camera_positions_x, camera_positions_y, camera_positions_z)),transform_matrix_first_frame,transform_matrix_last_frame


def filter_points_between(p1, p2, points):
    """
    Filter the points that are between p1 and p2.

    Parameters:
        p1 (np.ndarray): First 3D point.
...
# Generate synthetic data for demonstration
def generate_data():
    time = np.linspace(0, 10, 100)  # Generate time values
    x = 2 * np.sin(time)  # Replace with your own function
    y = 3 * np.cos(time)  # Replace with your own function
    z = np.random.uniform(-0.2, 0.2, len(time))  # Random z values in the range [-0.1, 0.1]
...
for t in np.linspace(0, 1, num_intermediate_frames + 2)[1:-1]:  # Exclude the endpoints (0 and 1)
    ...
    translation_vector_interpolate = (1 - t) * transform_matrix_first_frame[:3, 3] + t * transform_matrix_last_frame[:3, 3]
